...
const showDir = require('./etstatic/show-dir');
const version = require('../package.json').version;
const status = require('./etstatic/status-handlers');
const generateEtag = require('./etstatic/etag');
const optsParser = require('./etstatic/opts');
...
module.exports = function createMiddleware(_dir, _options) {
  ...
  let errorFileName = opts.errorFileName;
  ...
  return function middleware(req, res, next) {
    ...
    if (serverHeader !== false) {
      // Set common headers.
      res.setHeader('server', `etstatic-${version}`);
    }
        ...
        if (err && (err.code === 'ENOENT' || err.code === 'ENOTDIR')) {
          if (req.statusCode === 404) {
            ...
            // Try to serve default ./404.html
              if (!errorFileName) { errorFileName = 404; }
            middleware({
              url: (handleError ? `/${path.join(baseDir, `errorFileName.${defaultExt}`)}` : req.url),
              headers: req.headers,
              statusCode: 404,
            }, res, next);
          }
...
etstatic = module.exports;
etstatic.version = version;
etstatic.showDir = showDir;


if (!module.parent) {
  /* eslint-disable global-require */
  /* eslint-disable no-console */
  const defaults = require('./etstatic/defaults.json');
  const http = require('http');
  const minimist = require('minimist');
  const aliases = require('./etstatic/aliases.json');
  ...
  if (opts.help || opts.h) {
    console.error('usage: etstatic [dir] {options} --port PORT');
    console.error('see https://npm.im/etstatic for more docs');
  } else {
    http.createServer(etstatic(dir, opts))
      .listen(port, () => {
        console.log(`etstatic serving ${dir} at http://0.0.0.0:${port}`);
      })
    ;
  }
}
